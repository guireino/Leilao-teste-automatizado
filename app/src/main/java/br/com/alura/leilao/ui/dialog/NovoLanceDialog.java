...
public class NovoLanceDialog {
    ...
    public NovoLanceDialog(Context context, LanceCriadoListener listener, UsuarioDAO dao){
    ...
    private void mostraDialogUsuarioNaoCadastrado(){
        new AlertDialog.Builder(context).setTitle(USUARIOS_NAO_ENCONTRADOS).setMessage(MENSAGEM_NAO_EXISTE_USUARIOS_CADASTRADOS)
    ...
    private void configuraView(List<Usuario> usuarios){
        View viewCriada = LayoutInflater.from(context).inflate(R.layout.form_lance, null, false);
    ...
    private void mostraDialogPropoeLance(View viewCriada, Spinner campoUsuarios, EditText campoValor) {
        new AlertDialog.Builder(context).setTitle(TITULO).setView(viewCriada).setPositiveButton(DESCRICAO_BOTAO_POSITIVO,
                        criaNovoLanceListener(campoValor, campoUsuarios))
                .setNegativeButton(DESCRICAO_BOTAO_NEGATIVO, null).show();
    }
    ...
    private void configuraSpinnerUsuarios(List<Usuario> usuarios, Spinner usuariosDisponiveis) {
        SpinnerAdapter adapter = new ArrayAdapter<>(context, android.R.layout.simple_spinner_dropdown_item, usuarios);
